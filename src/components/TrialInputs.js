...
const SelectInput = ({ label, options, value, onChange}) => (
  <div className=" bg-transparent w-[560px] flex flex-col">
    <label className="block text-white font-mono text-[14px] ml-2 mb-1 p-1" htmlFor={label}>
      {label}
    </label>
    <select
...
const TrialInput = () => {
    const {subjects,documentType, setDocumentType,subject, setSubject, results}=useContext(Context)
  ...
  return (
    ...
    <div className='h-[1080px] w-[1200px] p-2  bg-cover object-center bg-no-repeat rounded-[100px] text-white flex flex-col justify-center items-center' style={{backgroundImage:`url(${img})`}}>
        <div className='text-[26px] font-bold p-3'>Results</div>
        <Line/>
        <div className='text-[26px] font-thin p-2'>Get your Documents Below</div>
{results.length===0?<h1 className='text-[26px] font-thin p-2'>No documents Available</h1>:<></>}
    ...
    <div className='flex flex-col h-full overflow-y-scroll no-scrollbar w-full'>
{
    ...
    <div className='flex justify-between'>
        ...
        <div className='flex flex-col'>
            <button><a href={`https://dbiiit.swoyam.engineer/download/${result._id}`}><Download/></a></button>
            <ShareLinkComponent link={`https://dbiiit.swoyam.engineer/download/${result._id}`}/>
